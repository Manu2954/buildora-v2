...
export default function AdminSidebar({
  ...
  const containerBase =
    "bg-card border border-border rounded-2xl shadow-sm h-full overflow-hidden";
  ...
  if (mobile) {
    return (
      <div className="fixed inset-0 z-50 md:hidden">
        ...
        <div
          className={`absolute left-0 top-0 bottom-0 w-64 ${containerBase} p-3`}
        >
          <div className="flex items-center justify-between px-2 py-1">
            ...
            <button
              onClick={onClose}
              aria-label="Close"
              className="h-8 w-8 inline-flex items-center justify-center rounded-xl border border-border bg-card"
            >
              <X className="h-4 w-4 text-buildora-text" />
            </button>
          </div>
            ...
            {items.map((it) => {
              ...
              return (
                <Link
                  ...
                  className={`flex items-center gap-3 rounded-xl px-3 py-2 min-h-[44px] text-sm ${active ? "bg-buildora-gold text-white" : "text-buildora-text hover:bg-muted"}`}
                >
  ...
  return (
    <aside
      id="admin-sidebar"
      className={`hidden md:block ${width} transition-all duration-200`}
    >
      <div
        className={`${containerBase} ${collapsed ? "p-2 flex items-center justify-center h-60" : "p-3"}`}
      >
        {!collapsed && (
        ...
        <nav
          className={`${collapsed ? "flex flex-col items-center justify-center gap-4" : "px-2 pb-2 space-y-1"}`}
